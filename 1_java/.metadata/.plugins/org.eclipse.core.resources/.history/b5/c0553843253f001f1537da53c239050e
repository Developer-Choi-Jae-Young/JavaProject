...
public class RequestRegistUserDao extends Functional{
	...
	public List<RequestRegistUser> selectRequestRegistUser() {
		...
		try {
			...
			while (rs.next()) {
				...
				userList.add(new RequestRegistUser(id, userId, name, password, age, gender, phone, email, type));
			}
	...
	public boolean insertRequestRegistUser(RequestRegistUser user) {
		...
		if (CallSqlOther(DataSource.Connect(),
				"INSERT INTO RequestRegistUser (userid, name, password, age, gender, phone, email, type) VALUES " + "(\"" + user.getUserId() + "\", \"" + user.getName() + "\", \"" + user.getPassword() + "\"," + user.getAge() + ", \'" + user.getGender() + "', \"" + user.getPhone() + "\", \"" + user.getEmail() + "\", \"" +  user.getType() + "\");") > 0) {
